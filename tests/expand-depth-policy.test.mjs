...
test("buildExpandDepthOptions includes depth 0 so users can expand only the root", () => {
  assert.deepEqual(buildExpandDepthOptions(0), [0]);
  assert.deepEqual(buildExpandDepthOptions(1), [0, 1]);
  assert.deepEqual(buildExpandDepthOptions(3), [0, 1, 2, 3]);
  /**
   * `maxDepth` 可能包含叶子层深度；展开层级只对容器节点有意义。
   * 当可展开容器最大深度小于总深度时，选项必须截断到容器深度上限，避免出现“选了没变化”的档位。
   */
  assert.deepEqual(buildExpandDepthOptions(4, 3), [0, 1, 2, 3]);
});
...
test("applyExpandByDepth accepts depth 0 instead of coercing it to depth 1", () => {
  const source = readAppSource();

  assert.match(
    source,
    /applyExpandByDepth\(depth\)[\s\S]*?Math\.max\(0, Math\.floor\(depth\)\)/,
  );
});
